...
<template>
    ...
    <div class="mx-auto w-full max-w-4xl my-10">
        ...
        <div class="w-full flex justify-center" v-if="Object.keys(productsData).length && productsData.products.length">
                ...
                <PaginationList v-slot="{ items }" class="flex items-center gap-1">
                    ...
                    <template v-for="(item, index) in items">
                        <PaginationListItem v-if="item.type === 'page'" :key="index" :value="item.value" as-child>
                            <Button class="w-10 h-10 p-0" :variant="item.value === page ? 'default' : 'outline'" @click="async () => {
